...
sąsiedztwa. 
Wzór dla jej obliczania został przedstawiony na równaniu \ref{eq:mov}.
Metoda jest często stosowana do ograniczania
krótkotrwałego szumu w danych sensorowych~\cite{bib:ioe1},~\cite{bib:ioe4},~\cite{bib:ioe5}.


\begin{equation}
\label{eq:mov}
M_t = \frac{1}{N} \sum X_i
\end{equation}
...
Filtr medianowy jest filtrem nieliniowym (wzór \ref{eq:med}). Nie
...
oknie kroczącym z medianą i estymatorem skali Sn (wzór \ref{eq:zsc}). Takie podejście
...
\begin{equation}
\label{eq:zsc}
Z_i = \frac{X_i - \text{mediana}(W_i)}{S_n(W_i)}
\end{equation}
...
kroczącym (wzór \ref{eq:iqr}). Dzięki temu granice outlierów dopasowują się do
...
\begin{aligned}
IQR &= Q_3 - Q_1 \\[1ex]
\text{dolna granica} &= Q_1 - 1.5 \cdot IQR \\[1ex]
\text{górna granica} &= Q_3 + 1.5 \cdot IQR
\end{aligned}
\end{equation}
...
jednowymiarowej użyto wzorów \ref{eq:kal}.


\begin{equation}
\label{eq:kal}
\begin{aligned}
x_{\mathit{pred}} &= x_{\mathit{poprz}} \\[1ex]
P_{\mathit{pred}} &= P_{\mathit{poprz}} + Q \\[1ex]
K &= \frac{P_{\mathit{pred}}}{P_{\mathit{pred}} + R} \\[1ex]
x_{\mathit{now}} &= x_{\mathit{pred}} + K \cdot (z - x_{\mathit{pred}})
\end{aligned}
\end{equation}
...
Zmienne takie jak liczba przesłanych bajtów czy czas trwania charakteryzują się skrajnymi wahaniami wartości, co paraliżuje działanie modeli liniowych (np. regresji logistycznej).
W celu normalizacji rozkładów zastosowano transformację logarytmiczną typu $\log(x + 1)$ (wzór~\ref{eq:sigma}).
\begin{equation}
\label{eq:sigma}
	x^* = \ln(x + 1)
\end{equation}
...
Pozostałe cechy numeryczne poddano klasycznej standaryzacji przy użyciu algorytmu \texttt{StandardScaler}~\cite{bib:scaler}, sprowadzając ich rozkłady do średniej równej zero oraz odchylenia standardowego równego jeden.
...
W celu oceny wpływu zaprojektowanego procesu przetwarzania danych na skuteczność wykrywania anomalii sieciowych, przeprowadzono testy porównawcze na czterech etapach:
\begin{itemize}
\item dla danych surowych z dodanymi etykietami
\item dla danych po oczyszczeniu i naprawie
\item dla danych zgrupowanych
\item dla danych zgrupowanych po procesie standaryzacji i skalowania wartości
\end{itemize}
We wszystkich przypadkach dane zostały również zgrupowane.

W eksperymentach zestawiono działanie dwóch algorytmów nadzorowanych: klasyfikatora histogramowego wzmacniania gradientu (\emph{HistGradientBoosting})~\cite{bib:histogramowe} oraz regresji logistycznej z regularyzacją $L_1$~\cite{bib:regresja}.
Zastosowany został również jeden algorytm nie-nadzorowany - las izolacyjny~\cite{bib:las}.
...
Świadczy to o silnym wpływie parametru zbalansowania wag klas, który zmusił model do kategoryzacji większości niejednoznacznych sesji jako potencjalnych ataków, minimalizując krytyczny z punktu widzenia bezpieczeństwa współczynnik fałszywych negatywów (FN spadające do poziomu kilkudziesięciu rekordów).
...
\begin{itemize}
    ...
    Zastosowanie zbalansowanych wag klas zminimalizowało liczbę fałszywych negatywów do krytycznie niskiego poziomu (jedynie 64 przeoczone ataki), kosztem akceptowalnego wzrostu fałszywych alarmów.
...
\bibitem{bib:histogramowe} https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.HistGradientBoostingClassifier.html. Dostęp 03.06.2026.
\bibitem{bib:regresja} https://scikit-learn.org/stable/modules/generated/sklearn.linear\_model.LogisticRegression.html. Dostęp 03.06.2026.
\bibitem{bib:las} https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.IsolationForest.html. Dostęp 03.06.2026.
\bibitem{bib:sigma} https://numpy.org/doc/stable/reference/generated/numpy.log1p.html. Dostęp 03.06.2026.
\bibitem{bib:scaler} https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.StandardScaler.html. Dostęp 03.06.2026.
